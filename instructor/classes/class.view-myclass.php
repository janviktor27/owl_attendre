		...
		if($count > 0){
			while($row=mysqli_fetch_array($sqlSearch)){
				...
				if($sqlCheck->num_rows > 0){
					$row_class = $sqlCheck->fetch_array();
					$_ID = $row_class['attend_id'];
					$dynamicCHK = "
					<a data-toggle='modal' data-target='#delMod$_ID' class='btn btn-danger btn-xs'><i class='fa fa-times'></i></a>";
				}else{
			...
			if($count > 0){
			  while($row=mysqli_fetch_array($sqlSearch)){
			    ...
			    if($sqlCheck->num_rows > 0){
						...
						$_ID = $row_class['attend_id'];
						echo"
						<div aria-hidden='true' id='delMod$_ID' aria-labelledby='myModalLabel' class='modal fade'  role='dialog' tabindex='-1'>
						  <div class='modal-dialog'>
						    <div class='modal-content'>
						      ...
						      <div class='modal-body'>
										<h3>This will delete the attendance mark.</h3>
						      </div>
		...
		//DELETE MODAL
			function delAction(){
				...
				if(isset($_POST['btn_del'])){
					...
					if($sqlDel){
						ob_end_clean();
						header("location: view.myclass.php?sched=$enc_sched_id&&subj=$enc_subj_id&&del=true");
					}else{
						ob_end_clean();
						header("location: view.myclass.php?sched=$enc_sched_id&&subj=$enc_subj_id&&del=false");
					}
				}
			}
